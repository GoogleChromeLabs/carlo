...
class Window {
  ...
  constructor(app, page, options) {
    ...
    this.initNetwork_ = page.target().createCDPSession().then(session => {
      ...
      return Promise.all([
        ...
        (testMode ? Promise.resolve() : session.send('Browser.getWindowForTarget', { targetId }))
  ...
  initBounds_(result) {
    this.windowId_ = result ? result.windowId : undefined;
  ...
  async requestIntercepted_(payload) {
    ...
    for (const [prefix, folders] of this.www_) {
      let pathname = decodeURIComponent(url.pathname.substr(1));
  ...
  async bounds() {
    ...
    if (!this.windowId_) {
      const viewport = this.page_.viewport();
  ...
  async setBounds(bounds) {
    // Testing mode support.
    if (!this.windowId_)
      return this.page_.setViewport({ width: bounds.width, height: bounds.height });
    await this.session_.send('Browser.setWindowBounds', { windowId: this.windowId_, bounds });
  }

  async fullscreen() {
    if (!this.windowId_)
      return this.page_.setViewport({ width: 1920, height: 1080 });
