  ...
  async init_() {
    ...
    await Promise.all([
      ...
      this.browser_.defaultBrowserContext().
          overridePermissions(`https://${this.options_.domain}`, [
            'geolocation',
            'midi',
            'notifications',
            'camera',
...
class Window extends EventEmitter {
  ...
  async load(uri = '', ...params) {
    ...
    await this.page_.goto(new URL(this.loadURI_, `https://${this.options_.domain}`).toString(), {timeout: 0, waitFor: 'domcontentloaded'});
  ...
  async handleRequest_(request) {
    ...
    if (url.hostname !== this.options_.domain) {
