...
class App extends EventEmitter {
   ...
   * @param {!Color} bgcolor
   */
  constructor(browser, bgcolor, options) {
    super();
  ...
  async init_() {
    ...
    this.pendingWindows_.set('', { bgcolor: this.bgcolor_ });
  ...
  async createWindow(options = {}) {
    const {
      width = this.options_.width || 800,
      height = this.options_.height || 600,
      bgcolor = null
    } = options;
    ...
    for (const page of this.windows_.keys()) {
      page.evaluate(`window.open('about:blank?seq=${seq}', '', 'width=${width},height=${height}')`);
      break;
    }

    return new Promise(callback => {
      this.pendingWindows_.set(seq, { bgcolor: bgcolor ? Color.parse(bgcolor) : this.bgcolor_,  callback });
    });
  }
  ...
  pageCreated_(page) {
    ...
    const { callback, bgcolor } = this.pendingWindows_.get(seq);
    this.pendingWindows_.delete(seq);
    window = new Window(this, page, bgcolor);
...
class Window {
    ...
    this.initNetwork_ = page.target().createCDPSession().then(session => {
      ...
      return Promise.all([
        ...
        session.send('Browser.getWindowForTarget', { targetId })
            .then(result => this.windowId_ = result.windowId)
      ]);
    });
  ...
  async initializeRpc_(params) {
    ...
    await this.page_.evaluate(rpcFile => {
      ...
      self.rpc.initWorld(transport, async params => {
        ...
        if (self.load)
          return self.load(params.userParams);
      });
    }, rpcFile);
    ...
    return rpc.createWorld(transport, { userParams: params, window: this.handle_ });
  }
   ...
   */
  async bounds() {
    const { bounds } = await this.session_.send('Browser.getWindowBounds', { windowId: this.windowId_ });
  ...
  async setBounds(bounds) {
    await this.session_.send('Browser.setWindowBounds', { windowId: this.windowId_, bounds });
  }
...
async function launch(options = {}) {
  ...
  await context.overridePermissions('https://domain', [
    ...
    'clipboard-read',
    'clipboard-write']);
  const app = new App(browser, bgcolor, options);
  await app.init_();
  return app;
}
