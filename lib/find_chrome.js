...
function darwin(canary) {
  const suffix = canary ? '/Contents/MacOS/Google Chrome Canary' : '/Contents/MacOS/Google Chrome';
...
function linux(canary) {
  ...
  return sort(uniq(installations.filter(Boolean)), priorities).pop();
}
...
async function downloadChromium(targetRevision) {
  const browserFetcher = puppeteer.createBrowserFetcher({ path: path.join(__dirname, '.local-data') });
...
async function findChrome(options) {
  ...
  for (const item of config) {
    if (!item.startsWith('r'))
      continue;
    const revisionInfo = await downloadChromium(item.substring(1));
    return { executablePath: revisionInfo.executablePath, type: revisionInfo.revision };
  }
